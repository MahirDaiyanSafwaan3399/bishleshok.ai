...
import { db, auth, signInUser, getDataCollectionRef } from "./firebase/config";
...
const API_KEY = "AIzaSyAWZnuxOqowvSm-oKjOicucQSXb94gAo9I";
...
function App() {
    ...
    const dropZoneRef = useRef(null); // For the drag & drop zone
        ...
        async (file) => {
            ...
            try {
                ...
                if (jsonText) {
                    ...
                    updateStatus(
                        `Successfully extracted data from ${file.name}.`,
                        false
                    );
                } else {
                    throw new Error(
...
ANSWER (in Markdown):`;
    ...
    const saveToExcel = useCallback(() => {
        ...
        updateStatus("Data exported successfully to CSV!", false);
    }, [extractedDataList, updateStatus]);
    ...
    const handleRowClick = useCallback(
        (index) => {
            ...
            setJsonOutput(JSON.stringify(extractedDataList[index], null, 2));
        },
        [extractedDataList]
    );
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                ...
                updateStatus(
                    `Data loaded (${data.length} items). Ready for input.`,
                    false
                );
                setJsonOutput(
                    "Select a row in the table to view raw JSON data here."
    ...
    // 4. Staggered fade-in animations for panels
    useEffect(() => {
    ...
    return (
        <div
            id="app-container"
            className="relative max-w-7xl mx-auto p-4 md:p-8 flex flex-col space-y-8"
        >
            <Header />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Dashboard data={extractedDataList} />
                <MarketTrends
